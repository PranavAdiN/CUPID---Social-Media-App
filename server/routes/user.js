...
recordRoutes.route("/user/all").get(function (req, res) {
...
recordRoutes.route("/user_update/:_id").post(function (req, response) {
 let db_connect = dbo.getDb();
...
recordRoutes.route("/user_delete/:id").delete((req, response) => {
 let db_connect = dbo.getDb();
 let myquery = { _id: ObjectId(req.params.id) };
